      ...
      <div className="container mx-auto px-4 lg:px-8 py-6 sm:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 xl:gap-20">
          ...
          >
              ...
              <motion.div variants={fadeUp} className="space-y-4 mb-6">
                ...
                <div ref={addToCartButtonRef} className="flex gap-3">
                  <button
                    ...
                    className="group relative flex-1 py-4 bg-obsidian-900 text-white text-xs uppercase tracking-[0.2em] font-medium overflow-hidden cursor-pointer active:scale-[0.98] transition-transform"
                  >
                    <span className="relative z-10">Agregar al Carrito</span>
                    <div className="absolute inset-0 bg-gradient-to-r from-amber-gold-600 to-amber-gold-500 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out" />
                  </button>
                  <button
                    onClick={handleToggleWishlist}
                    className={`w-14 h-auto flex items-center justify-center border-2 transition-all duration-300 cursor-pointer ${
                      isInWishlist
                        ? 'border-amber-gold-500 bg-amber-gold-50'
                        : 'border-pearl-300 hover:border-obsidian-900'
                    }`}
                    aria-label={isInWishlist ? 'Quitar de favoritos' : 'Agregar a favoritos'}
                  >
                    <svg
                      className={`w-5 h-5 transition-colors ${
                        isInWishlist ? 'text-amber-gold-500' : 'text-obsidian-700'
                      }`}
            ...
            {/* Out of stock */}
            {product.stock === 0 && (
              <motion.div variants={fadeUp} ref={addToCartButtonRef} className="mb-6">
                <button
                  disabled
                  className="w-full py-4 bg-platinum-300 text-platinum-600 text-xs uppercase tracking-[0.2em] font-medium cursor-not-allowed"
                >
                  Agotado
                </button>
              </motion.div>
