...
export default function AuthModal({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) {
  ...
  const handleLogin = async () => {
    try {
      ...
      loginStore(response.customer, response.access_token, response.refresh_token);
  ...
  const handleRegister = async () => {
    try {
      ...
      const message = error.response?.data?.message;
      if (typeof message === 'string' && message.includes('ya tiene')) {
        setErrors({ email: 'Este email ya tiene cuenta. ¿Iniciar sesión?' });
      } else {
        setErrors({ email: message || 'Error al crear la cuenta' });
      }
    }
  };
      ...
      google.accounts.id.initialize({
        ...
        callback: async (response: any) => {
          try {
            ...
            loginStore(result.customer, result.access_token, result.refresh_token);
