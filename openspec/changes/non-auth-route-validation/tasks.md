...
- [ ] 0.1 Confirm `zod` version in `package.json` supports `.int()` — **zod 4.4.3 installed; `.int()` is safe. No `.refine(Number.isInteger)` fallback needed.** (ADR S2-004)
- [ ] 0.2 Confirm slice-1 infra files exist unchanged: `app/lib/validation.ts` (validateBody) and `app/lib/bff-proxy.ts` (proxyToBackend with `body?` option). Do NOT modify. (ADR S2-001, S2-005)
...
- [ ] 1.1a Write failing tests in `app/lib/ecommerce/schemas.test.ts`:
...
- [ ] 1.2a Create `app/lib/ecommerce/schemas.ts` with `orderItemSchema`, `createOrderSchema`, `validateCouponSchema` per ADR S2-004 field shapes and `// Source:` comments. Run `pnpm test:run` — expect GREEN. (BFF-NAV-01, BFF-NAV-T5)

### 1.3 — Contact schemas (RED)

- [ ] 1.3a Write failing tests in `app/lib/contact/schemas.test.ts`:
...
- [ ] 1.4a Create `app/lib/contact/schemas.ts` with `createContactMessageSchema` per ADR S2-002 contact constraints. Run `pnpm test:run` — expect GREEN. (BFF-NAV-01, BFF-NAV-T5)

### 1.5 — Address schemas (RED)

- [ ] 1.5a Write failing tests in `app/lib/addresses/schemas.test.ts`:
...
- [ ] 1.6a Create `app/lib/addresses/schemas.ts` with `createAddressSchema` and `updateAddressSchema = createAddressSchema.partial()` per ADR S2-003. Run `pnpm test:run` — expect GREEN. (BFF-NAV-01, BFF-NAV-T5)
...
- [ ] 2.1a Write failing tests in `app/api/orders/route.test.ts`:
...
- [ ] 2.2b Run `pnpm test:run` — expect GREEN.

### 2.3 — POST /api/coupons/validate (RED)

- [ ] 2.3a Write failing tests in `app/api/coupons/validate/route.test.ts`:
...
- [ ] 2.4a Edit `app/api/coupons/validate/route.ts`: add `validateBody(req, validateCouponSchema)`; on failure return `v.response`; on success call `proxyToBackend(req, '/ecommerce/coupons/validate', { body: v.data })`. (BFF-NAV-03, BFF-NAV-07)
- [ ] 2.4b Run `pnpm test:run` — expect GREEN.

### 2.5 — POST /api/contact (RED)

- [ ] 2.5a Write failing tests in `app/api/contact/route.test.ts`:
...
- [ ] 2.6a Edit `app/api/contact/route.ts`: add `validateBody(req, createContactMessageSchema)`; on failure return `v.response`; on success call `proxyToBackend(req, '/contact', { body: v.data })`. (BFF-NAV-04, BFF-NAV-07)
- [ ] 2.6b Run `pnpm test:run` — expect GREEN.

### 2.7 — POST /api/addresses (RED)

- [ ] 2.7a Write failing tests in `app/api/addresses/route.test.ts` (POST handler only):
...
- [ ] 2.8a Edit `app/api/addresses/route.ts` POST handler: add `validateBody(req, createAddressSchema)`; on failure return `v.response`; on success call `proxyToBackend(req, '/ecommerce/me/addresses', { authenticated: true, body: v.data })`. GET handler unchanged. (BFF-NAV-05, ADR S2-005)
- [ ] 2.8b Run `pnpm test:run` — expect GREEN.

### 2.9 — PATCH /api/addresses/[id] (RED)

- [ ] 2.9a Write failing tests in `app/api/addresses/[id]/route.test.ts` (PATCH handler only):
...
- [ ] 2.10a Edit `app/api/addresses/[id]/route.ts` PATCH handler: add `validateBody(req, updateAddressSchema)`; on failure return `v.response`; on success call `proxyToBackend(req, \`/ecommerce/me/addresses/${id}\`, { authenticated: true, body: v.data })`. GET and DELETE handlers unchanged. (BFF-NAV-06, ADR S2-003, S2-005)
- [ ] 2.10b Run `pnpm test:run` — expect GREEN.

---

## Phase 3 — Full Suite Verification (sequential, after all Phase 2 tasks are GREEN)

- [ ] 3.1 Run `pnpm test:run` full suite — all existing tests plus new tests must pass; zero regressions. (BFF-NAV-T1)
- [ ] 3.2 Verify each schema file has one `// Source: <dto path> — <ClassName>` comment per schema (BFF-NAV-T5). Automated: `rg "// Source:" app/lib/ecommerce/schemas.ts app/lib/contact/schemas.ts app/lib/addresses/schemas.ts` — must emit 5 lines (one per schema).
- [ ] 3.3 Confirm deferred routes untouched: `app/api/orders/card-payment/route.ts`, `app/api/reviews/route.ts`, `app/api/reviews/[id]/helpful/route.ts` — no diff on these files. (BFF-NAV out-of-scope)
